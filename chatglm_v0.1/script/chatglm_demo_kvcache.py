...
class Pipeline:
    ...
    def load_model(self, seq_len):
        # load embedding
        self.embedding_model = SGInfer('../embedding_lmhead_bmodel/embedding_f16.bmodel', devices=[13])

        # load finallayernorm & lmhead
        self.lmhead_model = SGInfer('../embedding_lmhead_bmodel/lm_head_f16.bmodel', devices=[13])
        ...
        for i in range(14,28):
            block = SGInfer(f'../bmodel_{sys.argv[1]}/glm_block_{i}.bmodel', devices=[14])
            self.model_nokvcache_list.append(block)
    ...
    def model_runner(self, input_string, seq_len):
        ...
        for i in range(1):
            ...
            output_string = self.tokenizer.decode(next_token, skip_special_tokens=True, clean_up_tokenization_spaces=False) 
