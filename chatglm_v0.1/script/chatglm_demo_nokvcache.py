...
class Pipeline:
    ...
    def load_model(self, seq_len):
        # load embedding
        self.embedding_model = SGInfer('../embedding_lmhead_bmodel/embedding_f16.bmodel', devices=[11])

        # load finallayernorm & lmhead
        self.lmhead_model = SGInfer('../embedding_lmhead_bmodel/lm_head_f16.bmodel', devices=[11])

        # load nokvcache
        for i in range(28):
            block = SGInfer(f'../bmodel_{sys.argv[1]}/glm_block_{i}.bmodel', devices=[11])
    ...
    def preprocess_with_nokvcache(self, input_ids, seq_len, prompt_len):
        ...
        # mask
        if True:
            compact_mask = np.ones((valid_seq_len, valid_seq_len))
            compact_mask = np.tril(compact_mask)
            attention_mask = np.zeros((1, seq_len, seq_len))
            attention_mask[:, :valid_seq_len, :valid_seq_len] = compact_mask
            attention_mask[:, :prompt_len, :prompt_len] = 1
            attention_mask = (attention_mask < 0.5).astype(np.float32)
        else:
    ...
    def model_runner(self, input_string, seq_len):
        ...
        for i in range(30):
            # init & nokvcache
            model_input_ids, position_ids, block_position_ids, attention_mask = self.preprocess_with_nokvcache(input_ids, seq_len, prompt_len)
            ...
            # compute output string diff to avoid "I ' m"
            output_string = self.tokenizer.batch_decode([input_ids[-2], input_ids[-2:]], skip_special_tokens=True, clean_up_tokenization_spaces=False) 
            print(output_string[1][len(output_string[0]):], flush=True, end="")

            # stopping criterion
            if next_token == 130005:
                break
        return
...
print("==================================================================================================================")
print("A simple demo of chatglm-6b running on BM1684X. Prompts have no memories.")
print("==================================================================================================================")
