...
static const std::string CHATGLM_MODEL = "chatglm-6b.bmodel";
...
int ChatGLM::forward_first(std::vector<int> &tokens) {
  ...
  position_id[token_length - 2] = token_length - 2;
  position_id[token_length - 1] = token_length - 2;
  position_id[MAX_LEN + token_length - 1] = span_length; // block_position_id

  for (int i = 0; i < MAX_LEN; i++) {
    for (int j = 0; j < MAX_LEN; j++) {
      if (i < token_length - 1 && j < token_length - 1) {
      } else if (i == token_length - 1 && j < token_length) {
      } else {
...
int ChatGLM::forward_next() {
  ...
  int32_t position_id[2] = {token_length - span_length - 1, span_length};
  // embedding
...
void ChatGLM::answer(const std::string &input_str) {
  ...
  tokens.push_back(130001);
  tokens.push_back(130004);
  if (tokens.empty()) {
...
int main(int argc, char **argv) {
  ...
  printf("Demo for ChatGLM-6B in BM1684X\n");
  int dev_id = 0;
  if (argc > 1) {
    dev_id = atoi(argv[1]);
  }
