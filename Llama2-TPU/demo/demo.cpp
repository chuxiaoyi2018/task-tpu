...
static const int CACHE_MAX_LEN = 512;
static const int MAX_LEN = 513;
...
void LLama2::move2end(const bm_tensor_t &kv) {
  ...
  auto bytes = total_size / CACHE_MAX_LEN;
...
int LLama2::forward_first(std::vector<int> &tokens) {
  ...
  for (int i = 0; i < NUM_LAYERS; i++) {
    ...
    float test2[10] = {0};
    bm_memcpy_d2s(bm_handle, (void *)&test2, inputs_embed.device_mem);
...
int LLama2::forward_next() {
  float attention_mask[CACHE_MAX_LEN + 1] = {0};
  for (int i = 0; i <= CACHE_MAX_LEN - token_length; i++) {
...
int main(int argc, char **argv) {
  ...
  std::string llama_model = "llama2-7b.bmodel";
