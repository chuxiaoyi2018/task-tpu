...
class Block(torch.nn.Module):
    ...
    def forward(self, hidden_states, position_ids, attention_mask):
        ...
        return hidden_states, past_k[:,:,:MAX_LEN], past_v[:,:,:MAX_LEN]
...
def convert_block(layer_id):
    ...
    hidden_states = torch.randn((MAX_LEN + 1, 1, hidden_size))
    position_ids = torch.tensor([range(MAX_LEN + 1)], dtype=torch.long)
    attention_mask = -1000 * torch.ones((1, 1, MAX_LEN + 1, MAX_LEN + 1), dtype=torch.float32).triu(diagonal=1)
...
def test_net_with_mask():
    ...
    token_len = len(ids)
    ids = ids + (MAX_LEN + 1 - token_len) * [0]
    input_ids = torch.tensor(ids).view(MAX_LEN + 1)
    out = embed(input_ids).view(1, MAX_LEN + 1, hidden_size)
    position_ids = list(range(token_len)) + (MAX_LEN + 1 - token_len) * [0]
    position_ids = torch.tensor([position_ids])
    attention_mask = -1000 * torch.ones((MAX_LEN + 1, MAX_LEN + 1))
    for i in range(token_len):
        for j in range(token_len):
            if j <= i:
                attention_mask[i][j] = 0
    attention_mask = attention_mask.view(1, 1, MAX_LEN + 1, MAX_LEN + 1)
    k_cache = []
    v_cache = []
